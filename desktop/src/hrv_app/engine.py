...
class AnalysisEngine:
    ...
    def __init__(
        ...
        # v0.4.0：
        # UI 每 500 ms 刷新；研究原型只需要跟随约 20 s 的指标窗口更新。
        # 缓存一小时回顾结果，避免在相同 snapshot 上重复计算。
        self._research_cache_key: tuple[
            int,
            int,
        ] | None = None
        self._research_state_cache: dict = {}
        self._hour_experience_cache: dict = {}
    ...
    def reset(self) -> None:
        with self._lock:
            ...
            self._research_state_cache = {}
            self._hour_experience_cache = {}
    ...
    def _research_cache_locked(
        ...
        if (
            ...
            == cache_key
            and self._research_state_cache
            and self._hour_experience_cache
        ):
            return (
                copy.deepcopy(
                    self._research_state_cache
    ...
    def research_state(self) -> dict:
        ...
        该层只读取已经计算出的 HRV / 频域 / 质量历史，
        不回写检测器、RR cleaner 或 HRV 指标。
        """
        with self._lock:
            research_state, _ = (
                self._research_cache_locked()
            )
            return research_state

    def hour_experience(self) -> dict:
        """
        返回过去一小时会话层：阶段、状态分布、转场、持续时间和研究原型。
        """
        with self._lock:
            _, hour_experience = (
                self._research_cache_locked()
            )
            return hour_experience

    def research_state_table(self) -> list[dict]:
        return research_state_table()
