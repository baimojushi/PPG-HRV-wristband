...
from .literature_registry import sources_to_dict
from .models import (
    BeatFrame,
    DiagnosticFrame,
    FirmwareMetricFrame,
...
def export_engine_results(
    ...
    research_snapshot = bundle.get(
        "research_snapshot",
        {},
    )
    hour_experience = bundle.get(
        "hour_experience",
        {},
    )
    research_state_table = bundle.get(
    ...
    with (
        ...
        for beat in beats:
            writer.writerow([
                ...
                int(bool(getattr(beat, "firmware_unmatched", False))),
                int(bool(getattr(beat, "local_clipped", False))),
                str(getattr(beat, "artifact_class", "") or ""),
                float(getattr(beat, "artifact_confidence", 0.0) or 0.0),
                float(getattr(beat, "artifact_reference_rr_ms", 0.0) or 0.0),
                str(getattr(beat, "artifact_evidence", "") or ""),
                beat.flags,
            ])
    ...
    summary["research_prototypes"] = _json_safe(
        research_snapshot
    )
    summary["hour_experience"] = _json_safe(
        hour_experience
    )
    (
        destination
