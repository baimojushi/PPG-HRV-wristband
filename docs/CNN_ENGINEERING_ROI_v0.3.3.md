...
最新实机问题进一步说明：当前主要剩余误差来自宽峰内部 fiducial 漂移和节律相位窗口被上一搏误差拖动。

v0.3.4 已用独立相位跟踪、Peak Complex 合并和模板互相关直接处理。

因此运行时 CNN 当前仍为 NO-GO。

如果未来 CNN 只做 Candidate 真/假分类，它不能直接解决“真实心搏内部取哪个时间相位”的问题。

若采用 CNN fiducial 回归，需要同步 ECG R 峰或等价的独立高精度时间标签，并验证模型不会人为压缩 RR 变异。
