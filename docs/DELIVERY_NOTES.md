...
- 从最新实测定位主峰/次级同极性局部峰分支切换；
- branch transition 与 RR 异常显著共现；
- 固件延后 Winner 决策并将 morphology 权重提高到 90%；
- 模板首个相位只允许高分 Winner 建立；
- 增加宽范围相位恢复和独立节律一致性门；
- 导出 `timing_recovered`；
- UI 增加相位恢复计数；
- 协议保持 v4。
...
- UI 增加 F8 和“标记异常”按钮；
- 标签使用屏幕实际显示的设备 t_us；
- 保存 host monotonic 与 UI 数据滞后；
- 支持异常类型快速分类；
- PPG 图显示红色 -3~0 s 人工问题区间；
- annotations.csv 即时落盘；
- 导出完整 raw_session；
- 每个标签生成前120秒低频趋势；
- 检测和 HRV 算法不受人工标注反馈。
