# PPG / HRV 实时分析系统 v0.3.9


## v0.3.9 窗口分辨率自适应

本版在 v0.3.8 自动频域解析基础上补齐桌面窗口自适应：

- 状态页与专业分析页都进入原生 `QScrollArea`，内容超出可视高度时出现垂直滚动条；
- 专业分析页固定显示原生右侧滚动条，避免底部 Welch / SPWVD 被窗口裁掉；
- 窗口按宽度分为 `wide / medium / narrow` 三种布局，串口工具栏与人工标注栏自动换行；
- 极窄或极矮窗口自动折叠首页指标区，避免卡片挤压图表；
- 切换到“专业分析”时，“此刻 · 身体节律”自动缩成 72 px 极简条，只显示 HR / RMSSD / SQI / 频域状态；
- 图表使用“最小高度 + 页内滚动”，缩放窗口时不再把多个图压成重叠或不可读区域；
- resize 后重新同步 PPG 右轴和中位频率右轴 ViewBox，避免双轴图错位。
